...
function adminlogin($username,$password)
{
	...
	elseif($row['user_type']== '0')
	{
		...
		$_SESSION['email']=$row['email'];
		$_SESSION['user_type']=$row['user_type'];
		header('Location:dashboard.php?status=oki');
...
{
	global $con;
	$sql="DELETE FROM `xll_product` WHERE `product_id`='".mysqli_real_escape_string($con,$ProjectID)."' ";
...
{
	global $con;
	$sql="DELETE FROM `xll_faq` WHERE `faq_id`='".mysqli_real_escape_string($con,$faqID)."' ";
...
{
	global $con;
	$sql="DELETE FROM `coupons` WHERE `coupon_id`='".mysqli_real_escape_string($con,$coupon_id)."' ";
...
function changeorderstatusbydid($order_id,$status,$cart_id,$redir)
{
	...
	$sql="UPDATE `xll_order` SET `current_status`='".mysqli_real_escape_string($con,$status)."' WHERE `order_id`='".mysqli_real_escape_string($con,$order_id)."' ";
	if($con->query($sql))
	{
