...
function getname($table,$wh_field,$val,$return_index)
	{
				
		global $con;
		$result=mysqli_query($con,"select * from $table where $wh_field=$val");
		
		
		while($rows=mysqli_fetch_row($result))
		{
			return $rows[$return_index];
		}
		return 0;
	}
...
//manage coupons start here


function  manageAllcoupons()
{
	global $con;
	...
	if ($result->num_rows > 0) {
		 while($row = $result->fetch_assoc()) {
			 //getname("catrgories","category_id",$row['category_id'],1)
              
				                    
						echo " <tr>
				        <td class='center'>".$row['coupon_name']."</td>
				        <td class='center'>".$row['discount']."</td>
				        <td class='center'>".getname("catrgories","category_id",$row['category_id'],1)."</td>
				        <td class='center'>
            <a class='btn btn-danger' href='coupon.php?coupon_id=".base64_encode($row['coupon_id'])."'>
                <i class='glyphicon glyphicon-trash icon-white'></i>
                Delete
            </a>
...
//manage coupons ends here


// this function is to manage faq
function  managefaq()
{
	global $con;
	$sql="SELECT * FROM `xll_faq` ";
	$result = $con->query($sql);

	if ($result->num_rows > 0) {
		 while($row = $result->fetch_assoc()) {
		echo " <tr>
        <td class='center'>".$row['faq_id']."</td>
        <td class='center'>".$row['faq_question']."</td>
        <td class='center'>".$row['faq_answer']."</td>
        <td class='center'>
            <a class='btn btn-danger' href='faq.php?faqID=".base64_encode($row['faq_id'])."'>
                <i class='glyphicon glyphicon-trash icon-white'></i>
                Delete
            </a>

            <a class='btn btn-info' href='faq.php?EditID=".base64_encode($row['faq_id'])."&question=".base64_encode($row['faq_question'])."&answer=".base64_encode($row['faq_answer'])."' >
...
{
	global $con;
	$sql="SELECT `product_id`, `product_name`, `image`, `price`, `selling`, `special`,`SKU` FROM `xll_product` ";
	$result = $con->query($sql);
...
function ViewAccountHistory()
{
	...
	if ($result->num_rows > 0) {
		 while($row = $result->fetch_assoc()) {
		 	if($_SESSION['xllAdmin'] == $row['email'])
		 	{
			echo " <tr>
...
function viewpendingorder()
{
	...
	//sql
	$sql="SELECT `order_id`,`cart_id`,`order_by`,`totalamount`,`deliverycharge`,`mode`,`date`,`time` FROM `xll_order` WHERE `payment`='1' AND `current_status`='Pending' ORDER BY `order_id` DESC ";
	$result = $con->query($sql);
...
function viewproductitembycart($cart_id)
{
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$sql1="SELECT product_id,product_name,image,selling,SKU FROM `xll_product` WHERE `product_id`='".mysqli_real_escape_string($con,$row['product_id'])."'";
			$result1= $con->query($sql1);
			while($row1 = $result1->fetch_assoc())
			{
				echo "<tr>
...
{
	global $con;
	$sql="SELECT * FROM `xll_order` WHERE `order_id`='".mysqli_real_escape_string($con,$order_id)."' ";
	$result=$con->query($sql);
	if($result->num_rows>0)
	{
		while($row = $result->fetch_assoc()) {
...
{
	global $con;
	$sql="SELECT `order_id`,`cart_id`,`order_by`,`totalamount`,`deliverycharge`,`mode`,`date`,`time` FROM `xll_order` WHERE `payment`='1' ORDER BY `order_id` DESC ";
	$result = $con->query($sql);
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$total=$row['totalamount']+$row['deliverycharge'];
			echo "<tr>
			<td><a href='singleorder.php?orderid=".base64_encode($row['order_id'])."&cart_id=".base64_encode($row['cart_id'])."&redirct=order' >#ORDERXLL".$row['order_id']."</a></td>
			<td>".$row['order_by']."</td>
			<td>".$row['date']."</td>
			<td>".$row['time']."</td>
			<td>".$row['mode']."</td>
...
function viewpendingreview()
{
	...
	if ($result->num_rows > 0) {
		while($row=$result->fetch_assoc())
		{
			$sql1="SELECT `product_name`,`image` FROM `xll_product` WHERE `product_id`='".$row['product_id']."' ";
			$result1=$con->query($sql1);
